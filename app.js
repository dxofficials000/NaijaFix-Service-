...
async function initRotatingBanner() {
  const bannerSlider = document.getElementById('bannerSlider');
  if (!bannerSlider) return;

  const allArtisans = await getAllArtisans();
  const banners = allArtisans.filter(a => a.featured === true && a.active !== false);
...
// Combine static categories (data.js) with any new custom categories
// artisans have added via "Other" on their dashboard.
async function getAllCategories() {
  const staticCategories = dprosDatabase.getCategories();
  ...
  try {
    ...
    console.error('NaijaFix: could not load shared categories.', err);
    return staticCategories;
  }
}
